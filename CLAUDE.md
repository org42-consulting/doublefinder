...
DoubleFinder is a dual-pane macOS file-manager app written in SwiftUI + AppKit. It is a SwiftPM package with a single target — the `DoubleFinder` executable (`Sources/DoubleFinder`) — targeting macOS 26 with the Swift 6.2 toolchain in Swift 5 language mode. All Swift, no C. There is no test target; `swift build` is the verification step.
...
- Remote stack (`Sources/DoubleFinder/Remote/`) — `FileTransport` is the protocol every filesystem read/write goes through; `LocalFileTransport`, `SFTPFileTransport`, `WebDAVFileTransport`, and `FTPFileTransport` implement it. **`FileOps.transport(for:)` is the only place that maps a URL scheme to a transport** — `TabState.transport` delegates to it. Never add a second scheme switch: there used to be one here and one in `TabState`, they drifted (only `TabState`'s knew all four schemes), and the result was that browsing a WebDAV tab worked while every file operation on it silently fell through to `LocalFileTransport`. `RemoteSessionManager.shared` owns one `SFTPSession` (an actor driving `/usr/bin/sftp` over a pty via `PtyChannel`) per `(user, host, port)`, refcounted by tab. `RemoteServerStore.shared` persists bookmarks (`servers.json`, mode 0600) with passwords in Keychain. `RemoteEditWatcher.shared` runs the download → edit → re-upload loop. `RemoteEndpoint.supportedSchemes` is the single scheme/label/default-port table — the Connect sheet, the bookmark editor, and URL round-tripping all read it.
- `PtyChannel` — spawns `sftp(1)` on a pseudo-terminal using **`posix_spawn`, never `fork`**. This is deliberate and load-bearing: on Darwin `posix_spawn` is one system call, so no child process ever runs our code, and the rule that a child may only call async-signal-safe functions between `fork` and `exec` doesn't apply. Swift can't guarantee the compiler emits no runtime calls (a retain, an exclusivity check, an allocation) inside that window, which is why this used to be a C shim in a second SwiftPM target. Don't reintroduce `fork`/`forkpty` here — `forkpty` *is* callable from Swift, and that's exactly the trap. Two attributes replace `forkpty`'s `login_tty()`, and `ssh` needs both to prompt for passwords: `POSIX_SPAWN_SETSID` makes the child a session leader, and opening the slave pty **by path** as a spawn file action gives it a controlling terminal (a session leader with no ctty that opens a tty without `O_NOCTTY` acquires it). `POSIX_SPAWN_SETSIGDEF` resets inherited `SIG_IGN` dispositions, which is what keeps `interruptInFlight()`'s `^C` arriving as an ordinary SIGINT.
- `URL.isRemote` vs `URL.isRemoteSFTP` — **`isRemote` is almost always the one you want.** Use it for anything that asks "is this URL on another machine": every local-filesystem service (FSEvents, `git`, xattr tags, QuickLook thumbnails, Spotlight, `statfs`, `chmod`) will otherwise treat a remote URL's `path` component as a *local* path and quietly operate on the wrong file. Reserve `isRemoteSFTP` for genuinely SFTP-specific mechanisms: the `sftp(1)` session and its refcount, `ssh -t`, Edit Locally, and interrupting a transfer mid-file. Scheme-agnostic path arithmetic goes through `URL.parentDirectory` / `URL.childURL(named:)` rather than `deletingLastPathComponent()` / `appendingPathComponent()`, which leave trailing-slash artefacts on non-file URLs.
